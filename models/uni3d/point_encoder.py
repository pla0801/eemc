...
class PointcloudEncoder(nn.Module):
    ...
    def forward(self, pts, colors):
        ...
        if self.cache_type == 'global':
            ...
            patch_centers = self.visual.norm(patch_centers)
            patch_centers = self.visual.fc_norm(patch_centers)
            patch_centers = self.trans2embed(patch_centers)

            if self.cache_type == 'local':
                return patch_centers
            else:
                global_feat = self.visual.norm(x[:, 0, :])
                global_feat = self.visual.fc_norm(global_feat)
                global_feat = self.trans2embed(global_feat)
                
                if self.cache_type == 'hierarchical':
                    return global_feat, patch_centers
                else:   # NOTE for visualization purpose
                    all_patches = self.visual.norm(x[:, 1:, :])
                    all_patches = self.visual.fc_norm(all_patches)
                    all_patches = self.trans2embed(all_patches)
                    return global_feat, all_patches, patch_centers
